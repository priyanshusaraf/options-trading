...
Faithful port of the Pine v5 script. Everything the UI shows comes from here.
...
def to_payload(sig: pd.DataFrame, entry_z=1.0) -> dict:
    ...
    for _, r in sig.iterrows():
        ...
        # only entry signals on the chart (sparse, gated by the crossover);
        # raw exit flags fire on most bars, so they live in the panel instead.
        if r["longEntry"]:
