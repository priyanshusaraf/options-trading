...
MUTATIONS = (
    ("payload allowlist", ROOT / "app/backtest/public_computation.py",
     "if keys != _PUBLIC_RESULT_FIELD_SET:", "if False:",
     ["tests/test_public_backtest_computation.py::test_public_payload_requires_the_complete_v1_schema"]),
    ("manifest provenance rejection", ROOT / "app/backtest/public_computation.py",
     "if set(execution_manifest) != _REQUIRED_MANIFEST_KEYS:", "if False:",
     ["tests/test_public_backtest_computation.py::test_manifest_private_or_extra_key_never_queries_shared_artifact"]),
    ("payload semantic identity", ROOT / "app/backtest/public_computation.py",
     "if (payload.get(\"strategy_key\") != strategy_key", "if False and (payload.get(\"strategy_key\") != strategy_key",
     ["tests/test_public_backtest_computation.py::test_public_payload_semantics_must_match_the_artifact_identity"]),
    ...
    ("path-safe public address", ROOT / "app/backtest/dataset_store.py",
     "        _require_public(classification)\n        if not _is_public_address(address):\n            return None\n        started = dt.datetime.now().timestamp()",
     "        _require_public(classification)\n        started = dt.datetime.now().timestamp()",
     ["tests/test_dataset_store.py::test_public_address_rejects_malformed_values_before_any_path_or_database_io"]),
