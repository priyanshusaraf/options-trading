...
def _validate_binding_timestamp(value):
...
def _validate_binding_parameter(value, kind, minimum, maximum, values):
    if kind == "utc_timestamp":
        _validate_binding_timestamp(value)
    elif kind == "enum":
...
def _validate_binding_ports(inputs, port_fields):
    ...
    for port, fields in port_fields:
        ...
        if port != primary_port and fact["instrument"]["role"] != port:
...
def _binding_rule(name):
    ...
    def bind(parameters, inputs, _spec=(name, parameter_specs, port_fields, outputs, builder)):
        ...
        for key, kind, required, default, minimum, maximum, values in parameter_specs:
            _validate_binding_parameter(parameters[key], kind, minimum, maximum, values)
        primary_port = _validate_binding_ports(inputs, port_fields)
...
def _check_bound(name, parameters, bound):
    ...
    _check_bound_inputs(name, parameters, document["input_binding"]["ports"])
...
def _check_bound_inputs(name, parameters, ports):
    ...
    for port, fields in expected_ports.items():
        ...
        _check_bound_port(port, primary_port, fields, fact)


def _check_bound_port(port, primary_port, fields, fact):
    if (port != primary_port and fact["instrument"]["role"] != port or not set(fields) <= set(fact["fields"])
