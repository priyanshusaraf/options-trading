...
def _upgrade_user_graph_layouts() -> None:
    ...
    _rebuild("graph_versions", """
        CREATE TABLE __TABLE__ (
            ...
            FOREIGN KEY(owner_id,graph_identifier) REFERENCES graph_artifacts(owner_id,identifier) ON DELETE RESTRICT,
...
def _downgrade_refusal() -> None:
    bind = op.get_bind()
    names = _names()
    if "projects" in names and bind.execute(sa.text(
    ...
    if "graph_artifacts" in names and bind.execute(sa.text(
        "SELECT 1 FROM graph_artifacts GROUP BY identifier HAVING COUNT(*) > 1 LIMIT 1")).scalar() is not None:
...
def _downgrade_table(table: str, ddl: str, columns: str) -> None:
    temporary = f"{table}__0021"
    # A prior 0020 rollback may already have dropped its source and left a
    # completed `__0019` table for that revision's own recovery routine.
    if table not in _names():
        return
    _recover(table)
