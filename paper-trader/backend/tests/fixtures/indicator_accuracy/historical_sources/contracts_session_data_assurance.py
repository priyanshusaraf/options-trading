...
def input_binding_for_node(graph: Any, node: Any, source_contract: Mapping[str, Any],
    ...
    for port in source_contract["parameter_binding"]["input_ports"]:
        ...
        _require(source["binding"]["instrument"]["role"] == role, "canonical input role is swapped or mismatched")
...
def materialize_node_contract(source: Mapping[str, Any], registration: Any,
    ...
    from app.ir.registry import _DATA_ID, _validate_data_direct
    ...
    for port, entry in input_binding["ports"].items():
        ...
        role = "primary" if port == source["required_resolution"]["port"] else port
        _require(entry["binding"]["instrument"]["role"] == role, "node input role is swapped or mismatched")
    ...
    _require(isinstance(fields, (tuple, list)) and fields
             ...
             and set(fields) <= set(source["required_market_fields"]), "bound fields are undeclared or noncanonical")
