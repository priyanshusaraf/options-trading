...
INSTRUMENTS = ["NIFTY", "BANKNIFTY", "GOLDM"]
...
def test_parallel_rows_land_in_the_same_order_as_serial(store_root):
...
def test_a_pinned_parallel_run_makes_no_provider_call_and_matches_serial(store_root):
    ...
    serial_id = _sweep(cold, workers=1)
    serial = _artifacts(serial_id)
...
def test_no_worker_observes_another_workers_frame(store_root):
    ...
    try:
        _sweep(CountingMockProvider(), workers=2, instruments=["NIFTY"],
               intervals=["15minute"], strategies=STRATEGIES)
    finally:
...
def test_the_payload_that_crosses_the_boundary_carries_no_frame_and_no_session(
    ...
    for cell in payload["cells"]:
        assert cell["strategy_version"] and cell["params"]
...
def test_peak_memory_does_not_grow_with_the_number_of_cells(store_root):
    ...
    def peak_for(instruments):
        tracemalloc.start()
        tracemalloc.reset_peak()
        _sweep(provider, workers=2, instruments=instruments,
               intervals=INTERVALS, strategies=["trend_impulse_v3"])
        peak = tracemalloc.get_traced_memory()[1]
        tracemalloc.stop()
        return peak

    small = peak_for(["NIFTY"])                       # 2 datasets
    init_db(reset=True)
    large = peak_for(INSTRUMENTS)                     # 6 datasets, 3x the cells
