...
def _runner(key="NIFTY", product="equity_intraday", bar: dt.datetime | None = None,
    ...
    with r._session() as s:
        row = paper_authority.stage(s, project_id="test.admission.4c1029697ee358715d3a14a2", graph_identifier="test.strategy.expanding_z_impulse", graph_version=1, deployment_id=LEGACY_DEPLOYMENT_ID, instrument_key=key, interval="30minute", owner_id=r.owner_id, broker_account_id=r.broker_account_id); s.commit()
    with r._session() as s:
    ...
    r.publish_signal(
        key, r._binding_for(key), {"signal": signal, "z": 2.5, "slope": 1.0, "close": 100.0,
