...
def _broker(admitted_entry_identity):
    init_db(reset=True)
...
def test_partial_close_equity_long_keeps_invariant_and_splits_cost(admitted_entry_identity):
    b, admission = _broker(admitted_entry_identity)
    inst = get_instrument("NIFTY")
    pos = b.open_equity_position(inst, "LONG", 100.0, 100, "NSE_INTRADAY", "t", _now(), params={}, **admission)
...
def test_partial_close_equity_short_profits_when_price_falls(admitted_entry_identity):
    b, admission = _broker(admitted_entry_identity)
    inst = get_instrument("NIFTY")
    pos = b.open_equity_position(inst, "SHORT", 100.0, 100, "NSE_INTRADAY", "t", _now(), params={}, **admission)
...
def test_partial_close_equity_short_loses_when_price_rises(admitted_entry_identity):
    b, admission = _broker(admitted_entry_identity)
    inst = get_instrument("NIFTY")
    pos = b.open_equity_position(inst, "SHORT", 100.0, 100, "NSE_INTRADAY", "t", _now(), params={}, **admission)
...
def test_partial_then_full_remainder_stays_invariant(admitted_entry_identity):
    b, admission = _broker(admitted_entry_identity)
    inst = get_instrument("NIFTY")
    pos = b.open_equity_position(inst, "LONG", 100.0, 100, "NSE_INTRADAY", "t", _now(), params={}, **admission)
