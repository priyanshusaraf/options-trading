...
def _broker(admitted_entry_identity) -> tuple[PaperBroker, dict]:
    init_db(reset=True)
...
def test_open_equity_books_margin_not_full_notional(admitted_entry_identity):
    b, admission = _broker(admitted_entry_identity)
    cash0 = b.cash()
...
def test_equity_entry_charges_use_intraday_segment(admitted_entry_identity):
    b, admission = _broker(admitted_entry_identity)
    pos = b.open_equity_position(get_instrument("NIFTY"), "LONG", 1000.0, 50,
                                 "NSE_INTRADAY", "t", NOW, params=PARAMS, **admission)
    assert pos.entry_charges == pytest.approx(
        compute_charges("NSE_INTRADAY", "BUY", 1000.0, 50)["total"])


def test_close_equity_long_profit_reconciles(admitted_entry_identity):
    b, admission = _broker(admitted_entry_identity)
...
def test_close_equity_short_profits_when_price_falls(admitted_entry_identity):
    b, admission = _broker(admitted_entry_identity)
...
def test_close_equity_short_loss_when_price_rises(admitted_entry_identity):
    b, admission = _broker(admitted_entry_identity)
    pos = b.open_equity_position(get_instrument("NIFTY"), "SHORT", 100.0, 200,
                                 "NSE_INTRADAY", "t", NOW, params=PARAMS, **admission)
...
def test_portfolio_equity_is_margin_plus_pnl_not_full_notional(admitted_entry_identity):
    ...
    cap = analytics_capital_dict(b.s, owner_id="owner", broker_account_id="account.default")
