...
def _short(admitted_entry_identity):
    init_db(reset=True)
    ...
    pos = r.broker.open_equity_position(get_instrument("NIFTY"), "SHORT", ENTRY, QTY,
                                        SEG, "t", r.provider.now(), params={}, **admission)
    return r, pos


def test_short_entry_leg_is_charged_as_a_sell(admitted_entry_identity):
    r, pos = _short(admitted_entry_identity)
...
def test_short_round_trip_charges_match_the_real_order_sequence(admitted_entry_identity):
    r, pos = _short(admitted_entry_identity)
...
def test_long_legs_are_unchanged(admitted_entry_identity):
...
def test_short_partial_close_covers_with_a_buy_leg(admitted_entry_identity):
    r, pos = _short(admitted_entry_identity)
...
def test_ledger_stays_exact_for_a_short_round_trip(admitted_entry_identity):
