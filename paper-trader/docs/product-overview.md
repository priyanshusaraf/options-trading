...
| **Reliability** | 6 | Two-lane async design, heartbeats, a staleness watchdog, ledger-drift alarms, an instance lock, rollback discipline, and paisa-exact reconciliation, plus a completed 26-finding hardening pass. Docked for single-process/local-DB design and documented stale-process operational hazards. (Corrected 2026-07-28: this previously also docked the score for "the hardened code is not yet deployed" — that was false; Workstream B and E0/E1 were verified live on the VPS. Score not re-raised here, as the remaining deductions were not re-weighed.) |
...
| **Production readiness** | 3 | The paper path is solid, but the live order path has never fired, slippage is unmeasured, operation is single-account with manual daily re-auth, and unattended running is not yet supervised. |
