...
- Modify: `paper-trader/backend/app/ir/validate.py`
- Modify: `paper-trader/backend/app/ir/resolve.py`
- Modify: `paper-trader/backend/app/ir/registry.py`
- Modify: `paper-trader/backend/app/ir/runtime.py`
- Modify: `paper-trader/backend/app/ir/streaming_reference.py`
...
- Test: `paper-trader/backend/tests/test_ir_conformance.py`
- Test: `paper-trader/backend/tests/test_ir_resolution.py`
- Test: `paper-trader/backend/tests/test_ir_platform_library.py`
- Test: `paper-trader/backend/tests/test_ir_runtime.py`
- Test: `paper-trader/backend/tests/test_ir_streaming_reference.py`
...
Before running the gate, close the narrow v1 edge-safety defect. With the existing v1 socket
grammar and component library, validation accepts only interface-input or node-output sources and
node-input or interface-output targets. Resolution and registry construction independently retain
that library-aware direction refusal. Validation also refuses a second edge to an already-fed
target socket. `runtime.evaluate` and `streaming_reference.evaluate_prefix_stream` independently
refuse duplicate targets before building incoming-edge dictionaries, so a validation bypass cannot
obtain last-write-wins behavior. They do not recheck direction because `ResolvedGraph` does not
carry the component-interface map needed to classify endpoints. Add focused mutation tests for
direction in validation/resolution/registry and duplicate targets in validation/vector/prefix; each
test must go red when its own defense is locally removed. Do not introduce Port objects,
connection bounds, compatibility sets, semantic flows, or any other v2 claim.

Close the existing v1 graph-component boundary at the same gate. Resolution and admission must
refuse a published graph component when its interface differs from the referenced graph body's
interface. Validation and resolution must require exactly one producer for every declared graph
output, refusing both zero and more than one. Add focused RED tests and the four named boundary
mutations for interface mismatch in resolution/admission and missing output production in
validation/resolution; the existing duplicate-target tests cover the more-than-one case. This is
exact equality and producer completeness for the existing v1 interface. It does not add v2 Port
fields, generalized cardinality, compatibility, semantic flows, or v2 compound substitutability.

- [ ] **Step 4: Run focused SQLite closure**

Run: `cd paper-trader/backend && pytest -q tests/test_ir_conformance.py tests/test_ir_resolution.py tests/test_ir_platform_library.py tests/test_ir_runtime.py tests/test_ir_causal_contract.py tests/test_ir_implementation_identity.py tests/test_strategy_admission.py tests/test_ir_streaming_reference.py tests/test_causal_admission_mutations.py tests/test_strategy_admission_repository.py tests/test_ir_edit_routes.py tests/test_backtest_admission.py tests/test_strategy_admission_authority.py tests/test_execution_admission_attribution.py tests/test_strategy_admission_backfill.py research_tests/test_composition_ir.py research_tests/test_strategy_admissions.py research_tests/test_graph_experiment.py`
...
**Status:** future / not started; approved design handoff; current terminal condition is Phase 3
only.

Do not add Component IR v2 Port fields, a format dispatcher, generalized cardinality or
compatibility, semantic-flow compound substitutability, dynamic branch semantics, a v1 read-only
migration, or a v1-to-v2 upgrader to Task 12. Those are explicit nonclaims of Phase 3 and begin
only in the approved future Interphase 3 → 4 Port-contract handoff. This deferral does not remove
Task 12's narrow v1 published-interface-to-body equality or exactly-one graph-output producer
acceptance rules.

- [ ] **Step 9: Commit**

```bash
git add paper-trader/backend/app/ir/validate.py paper-trader/backend/app/ir/resolve.py paper-trader/backend/app/ir/registry.py paper-trader/backend/app/ir/runtime.py paper-trader/backend/app/ir/streaming_reference.py paper-trader/backend/scripts/phase3_causal_gate.py paper-trader/backend/tests/test_phase3_causal_gate.py paper-trader/backend/tests/test_ir_conformance.py paper-trader/backend/tests/test_ir_resolution.py paper-trader/backend/tests/test_ir_platform_library.py paper-trader/backend/tests/test_ir_runtime.py paper-trader/backend/tests/test_ir_streaming_reference.py paper-trader/docs/operations/strategy-admission.md paper-trader/docs/reports/2026-08-13-phase3-causal-strategy-contract.md paper-trader/docs/reports/phase3-causal-gate.json paper-trader/docs/ROADMAP.md paper-trader/docs/CONTINUE.md
git commit -m "fix(ir): close Phase 3 v1 edge safety"
```
