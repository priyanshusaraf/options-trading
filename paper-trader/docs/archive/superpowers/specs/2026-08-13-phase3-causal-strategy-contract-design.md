...
**IR v2 status:** future / not started; approved design handoff; current terminal condition is Phase 3 only.
...
- Narrow Component IR v1 edge safety: library-aware validation, resolution, and registry closure
  refuse reversed source/target direction. Validation also refuses more than one producer for the
  same target socket; vector and prefix-reference runtimes independently refuse duplicate targets
  before constructing incoming-edge maps. Resolved runtime plans do not retain the component
  interface map needed to reclassify edge direction, so no runtime-direction claim is made.
- Narrow Component IR v1 graph-component boundaries: a published graph component's existing
  interface must exactly equal its graph body's interface, and each declared graph output must
  have exactly one producer. Interface mismatch, a missing output producer, and a second output
  producer refuse before evaluation.
...
- Component IR v2 Ports, richer Port/cardinality/compatibility and semantic-flow compound
  contracts, runtime-selected fixed branches, v1 freezing, or v1-to-v2 conversion. This deferral
  does not remove Phase 3's narrow v1 requirement that the existing published graph-component
  interface equal its body interface and that each declared graph output have exactly one
  producer. The v2
  structural extension is future / not started and begins only after this phase's Task 12 closure;
  the current terminal condition is Phase 3 only. It has its own approved design handoff and plan.
  Phase 3's v1 socket grammar remains the sole grammar this phase admits.
...
- Reversed v1 edge: library-aware validation, resolution, and registry closure refuse it before a
  resolved plan reaches evaluation. Duplicate target: validation refuses it, and vector plus
  prefix-reference runtimes independently refuse before building an incoming-edge map if a caller
  bypasses validation. No runtime may let last-write-wins choose a producer. Resolved runtimes do
  not claim an independent direction check because `ResolvedGraph` carries no component-interface
  direction map.
- V1 graph-component boundary mismatch: resolution and admission refuse when a published component
  interface differs from its body interface. Validation and resolution refuse a declared graph
  output without exactly one producer. These checks use the existing v1 interface; they do not
  imply v2 Port, cardinality, compatibility, or semantic-flow support.
...
21. `expanding_z_v4` adapter admission passes. `trend_impulse_v3` is reported as quarantined, not
    silently admitted.
22. For v1 only, interface-input or node-output sources may feed node-input or interface-output
    targets in their existing legal shapes; library-aware validation, resolution, and registry
    closure each reject reversed directions before evaluation. A target socket has at most one
    producer: validation, vector runtime, and prefix-reference runtime independently kill the
    duplicate-target mutation. This is a narrow v1 safety closure and makes no runtime-direction,
    v2 Port, cardinality, compatibility, or semantic-flow claim.
23. For every graph-backed v1 component, the published interface exactly equals the graph body's
    declared interface, and every declared graph output has exactly one producer. Focused tests
    reject an interface mismatch, a missing output producer, and a second output producer. This is
    equality of the existing v1 interface, not the richer v2 compound Port contract.
